...
import { IoCalendar, IoBook } from 'react-icons/io5'
...
export default function ProfileDialog({ setisDialogOpen }) {
    ...
    useEffect(() => {
        const fetchReport = async () => {
            ...
            setLoading(true)
            try {
                const reportRes = await getAttendanceReport(user.userID, user.semester)
                ...
                const courseList = normalized.map((r) =>
                    r.courseName ? `${r.courseCode} - ${r.courseName}` : r.courseCode
                )
    ...
    const handleChangeSem = async () => {
        ...
        try {
            ...
            toast.error(error.message || 'Failed to update semester', { className: 'toast-error' })
            setLoading(false)
        }
    }
    ...
    const cancelEditCourse = (index, courseCode) => {
        const orig = originalAttendance[courseCode] || { present: 0, absent: 0, medical: 0 }
        setAttendanceReport((prev) => {
            const copy = [...prev]
    ...
    const saveCourse = async (courseObj) => {
        ...
        try {
            const res = await updateAttendance(user.userID, user.semester, courseObj.courseCode, {
            ...
            toast.success('Course attendance saved', { className: 'toast-success' })
            setEditingCourse(null)
        } catch (err) {
            toast.error(err.message || 'Failed to save', { className: 'toast-error' })
        } finally {
    ...
    const saveAllCourses = async () => {
        ...
        try {
            for (const c of attendanceReport) {
                const orig = originalAttendance[c.courseCode] || { present: 0, absent: 0, medical: 0 }
                if (
                    Number(c.present) !== Number(orig.present) ||
                    Number(c.absent) !== Number(orig.absent) ||
                    Number(c.medical) !== Number(orig.medical)
                ) {
                    await updateAttendance(user.userID, user.semester, c.courseCode, {
            ...
            attendanceReport.forEach((r) => {
                newOrig[r.courseCode] = { present: r.present, absent: r.absent, medical: r.medical }
            })
            setOriginalAttendance(newOrig)

            toast.success('All courses saved', { className: 'toast-success' })
        } catch (err) {
            toast.error(err.message || 'Failed to save all', { className: 'toast-error' })
        } finally {
            setLoading(false)
            setEditingCourse(null)
        }
                                ...
                                <div className="flex flex-col flex-1 w-full justify-center">
                                    ...
                                    <input
                                        ...
                                        className={`w-[4rem] text-[1.2rem] md:text-[1.6rem] text-right ${
                                            edit ? 'bg-white rounded-lg' : 'border-none'
                                        }`}
                                    />
                                </div>
                                {!edit && (
                                ...
                                <div className="flex flex-col gap-[0.5rem]">
                                    {loading ? (
                                        ...
                                        <>
                                            {!coursesExpanded && (
                                                <div className="flex flex-col gap-[0.5rem]">
                                                    {courses.map((course, index) => (
                                                        <p key={index} className="text-[1.2rem] md:text-[1.6rem]">
                                                            {course}
                                                        </p>
                                                    ))}
                                                </div>
                                            ...
                                            {coursesExpanded && attendanceReport.length === 0 && (
                                                <p className="text-[1.2rem]">No course attendance data available.</p>
                                            )}
                                            ...
                                            {coursesExpanded && attendanceReport.length > 0 && (
                                                <div className="flex flex-col gap-[1rem]">
                                                    ...
                                                    <div className="hidden md:flex justify-between items-center px-[0.5rem]">
                                                        <div className="w-1/3"><h4 className="font-semibold">Course</h4></div>
                                                    ...
                                                    {attendanceReport.map((c, idx) => (
                                                        <div key={c.courseCode} className="flex flex-col md:flex-row justify-between items-center bg-white p-[0.75rem] rounded-lg">
                                                            <div className="flex items-center gap-[0.5rem] w-full md:w-1/3">
                                                                <button
                                                                    onClick={() => startEditCourse(c.courseCode)}
                                                                    disabled={editingCourse === c.courseCode}
                                                                    className={`bg-secondary p-[0.4rem] rounded-lg ${editingCourse === c.courseCode ? 'opacity-50 cursor-not-allowed' : 'hover:bg-black'}`}
                                                                >
                                                            ...
                                                            <div className="flex-1 w-full md:w-2/3 flex gap-[0.5rem] justify-between items-center mt-[0.5rem] md:mt-0">
                                                                <input
                                                                    ...
                                                                    min={0}
                                                                    value={c.present}
                                                                    onChange={(e) => onChangeCourseValue(idx, 'present', e.target.value)}
                                                                    disabled={editingCourse !== c.courseCode}
                                                                ...
                                                                <input
                                                                    ...
                                                                    min={0}
                                                                    value={c.absent}
                                                                    onChange={(e) => onChangeCourseValue(idx, 'absent', e.target.value)}
                                                                    disabled={editingCourse !== c.courseCode}
                                                                    className="w-[4.5rem] text-[1rem] md:text-[1.2rem] text-right bg-white rounded-lg p-[0.25rem]"
                                                                />
                                                                <input
                                                                    ...
                                                                    min={0}
                                                                    value={c.medical}
                                                                    onChange={(e) => onChangeCourseValue(idx, 'medical', e.target.value)}
                                                                    disabled={editingCourse !== c.courseCode}
                                                                    className="w-[4.5rem] text-[1rem] md:text-[1.2rem] text-right bg-white rounded-lg p-[0.25rem]"
                                                                />
                                                                ...
                                                                <div className="flex gap-[0.5rem] ml-[1rem]">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        disabled={savingCourseId === c.courseCode}
                                                    ...
                                                    <div className="flex justify-end mt-[0.5rem]">
                                                        <button
                                                            ...
                                                            disabled={loading}
                                                            className={`bg-primary py-[0.6rem] px-[1rem] rounded-lg cursor-pointer hover:opacity-90 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                        >
                                                            <h4 className="uppercase">Save All</h4>
                                                        </button>
                                                    </div>
                                                </div>
                                            )}
                        ...
                        <div className="flex flex-col w-full bg-[#d7e4ee] justify-center items-center mt-[4rem] p-[1rem] gap-[1rem] rounded-lg">
                            ...
                            <button
                                className={`w-full flex justify-center items-center bg-red-300 p-[1rem] gap-[1rem] rounded-lg cursor-pointer ${
                                    buttonClicked ? 'opacity-50 ' : 'hover:bg-red '
                                }`}
                                onClick={async () => {
